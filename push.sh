...
if [ ! -z "$cas_version" ]
  then
  	...
  	echo "Pushing CAS docker image tagged as v$cas_version to apereo/cas-management..."
	docker push apereo/cas-management:"v$cas_version" && echo "Pushed CAS image successfully tagged as v$cas_version via user $docker_user";
  else
